#Name: Ethan Nguyen
#Class: 6th Hour
#Assignment: Semester Project 1

import random
import time

#Due to weird time travelling circumstances beyond explanation, you find yourself in 2018 or so
#working for Larian Studios. Currently, they are working on the early prototypes of the hype
#upcoming game "Baldur's Gate 3". BG3 is a game that uses the Dungeons & Dragons 5th edition rules
#as its framework for gameplay. You have been given a basic dictionary of some of the main hero
#characters and some of the enemies they may face, and are tasked with making an early prototype
#of one of the party members fighting against an enemy until one of them hits zero HP (dies).

partyDict = {
    "LaeZel" : {
        "HO" : 48,
        "Init" : 1,
        "AC" : 17,
        "AtkMod": 6,
        "Damage" : random.randint(1,6) + random.randint(1,6) + 3
    },
    "Shadowheart" : {
        "HP" : 40,
        "Init" : 1,
        "AC" : 18,
        "AtkMod": 4,
        "Damage" : random.randint(1,6) + 3,
    },
    "Gale" : {
        "HP" : 32,
        "Init" : 1,
        "AC" : 14,
        "AtkMod": 6,
        "Damage" : random.randint(1,10) + random.randint(1,10),
    },
    "Astarion" : {
        "HP" : 40,
        "Init" : 3,
        "AC" : 14,
        "AtkMod": 5,
        "Damage" : random.randint(1,8) + random.randint(1,6) + 4,
    }
}

enemyDict = {
    "Goblin" : {
        "HP" : 7,
        "Init" : 0,
        "AC" : 12,
        "AtkMod": 4,
        "Damage" : random.randint(1,6) + 2
    },
    "Orc": {
        "HP" : 15,
        "Init": 1,
        "AC" : 13,
        "AtkMod": 5,
        "Damage" : random.randint(1,12) + 3
    },
    "Troll": {
        "HP" : 84,
        "Init": 1,
        "AC" : 15,
        "AtkMod": 7,
        "Damage" : random.randint(1,6) + random.randint(1,6) + 4
    },
    "Mindflayer": {
        "HP" : 71,
        "Init": 1,
        "AC" : 15,
        "AtkMod": 7,
        "Damage" : random.randint(1,10) + random.randint(1,10) + 4
    },
    "Dragon": {
        "HP" : 127,
        "Init": 2,
        "AC" : 18,
        "AtkMod": 7,
        "Damage" : random.randint(1,10) + random.randint(1,10) + random.randint(1,8) + 4
    },
}

#Combat consists of these steps:

#1. Rolling for 'initiative' to see who goes first. This is determined by rolling a
#20-sided die (d20) and adding their initiative modifier (If the roll is the same,
#assume the hero goes first).



Player_ini = random.randint(1, 20) + partyDict["LaeZel"]["Init"]
print(f"The player rolled a {Player_ini}"),time.sleep(0.6)

enemy_ini = random.randint(1, 20) + enemyDict["Orc"]["Init"]
print(f"The enemy rolled a {enemy_ini}"),time.sleep(0.6)

#2. Rolling to attack. This is determined by rolling a 20-sided die (d20) and adding their
#attack modifier. The attack hits if it matches or is higher than the target's Armor Class (AC).
#If the d20 rolled to attack is an unmodified ("natural") 20, the attack automatically hits and
#the character deals double damage. If the d20 rolled to attack is an unmodified ("natural") 1,
#the attack automatically misses


if Player_ini > enemy_ini:
    print("The player goes first")
    while enemyDict["Orc"]["HP"] > 0 or partyDict["LaeZel"]["HO"] > 0:
        time.sleep(0.6)
        player_Atk = random.randint(1, 20) + partyDict["LaeZel"]["AtkMod"]
        enemy_Atk = random.randint(1, 20) + enemyDict["Orc"]["AtkMod"]

        if enemyDict["Orc"]["HP"] <= 0:
            print("The player has won")
            break
        elif partyDict["LaeZel"]["HO"] <= 0:
            print("The player has lost")
            break

        if player_Atk == 1:
            time.sleep(0.6)
            print("The player got a natural 1")
            print("player misses")

        elif player_Atk == 20:
            time.sleep(0.6)
            print("The player got a natural 20")
            time.sleep(0.6)
            player_DamageN20 = (random.randint(1,6) + random.randint(1,6) + 3) * 2
            print(f"The player deals {player_DamageN20} damage")
            enemyDict["Orc"]["HP"] -= player_DamageN20
            print(f"enemy's Hp is {enemyDict["Orc"]["HP"]}")

        elif player_Atk > enemyDict["Orc"]["AC"]:
            time.sleep(0.6)
            print(f"Player rolled {player_Atk}")
            print("The player hits")
            time.sleep(0.6)
            player_Damage = random.randint(1,6) + random.randint(1,6) + 3
            print(f"The player deals {player_Damage} damage")
            enemyDict["Orc"]["HP"] -= player_Damage
            print(f"enemy's Hp is {enemyDict["Orc"]["HP"]}")

        elif player_Atk < enemyDict["Orc"]["AC"]:
            time.sleep(0.6)
            print(f"Player rolled {player_Atk}")
            print("The player missed")

        if enemyDict["Orc"]["HP"] <= 0:
            print("The player has won")
            break
        elif partyDict["LaeZel"]["HO"] <= 0:
            print("The player has lost")
            break


        if enemy_Atk == 1:
            time.sleep(0.6)
            print("The enemy got a natural 1")
            print("The enemy misses")

        elif enemy_Atk == 20:
            time.sleep(0.6)
            print("The enemy got a natural 20")
            time.sleep(0.6)
            enemy_DamageN20 = (random.randint(1,10) + random.randint(1,10) + random.randint(1,8) + 4) * 2
            print(f"The enemy deals {enemy_DamageN20} damage")
            partyDict["LaeZel"]["HO"] -= enemy_DamageN20
            print(f"Player's Hp is {partyDict["LaeZel"]["HO"]}")

        elif enemy_Atk > partyDict["LaeZel"]["AC"]:
            time.sleep(0.6)
            print(f"Enemy rolled {enemy_Atk}")
            print("The enemy hits")
            time.sleep(0.6)
            enemy_Damage = random.randint(1,10) + random.randint(1,10) + random.randint(1,8) + 4
            print(f"The enemy deals {enemy_Damage} damage")
            partyDict["LaeZel"]["HO"] -= enemy_Damage
            print(f"Player's Hp is {partyDict["LaeZel"]["HO"]}")

        elif enemy_Atk < partyDict["LaeZel"]["AC"]:
            time.sleep(0.6)
            print(f"Enemy rolled {enemy_Atk}")
            print("The enemy missed")



elif enemy_ini > Player_ini:
    print("enemies go first")
    while enemyDict["Orc"]["HP"] > 0 or partyDict["LaeZel"]["HO"] > 0:
        time.sleep(0.6)
        player_Atk = random.randint(1, 20) + partyDict["LaeZel"]["AtkMod"]
        enemy_Atk = random.randint(1, 20) + enemyDict["Orc"]["AtkMod"]

        if enemyDict["Orc"]["HP"] <= 0:
            print("The player has won")
            break
        elif partyDict["LaeZel"]["HO"] <= 0:
            print("The player has lost")
            break

        if enemy_Atk == 1:
            time.sleep(0.6)
            print("The enemy got a natural 1")
            print("The enemy misses")

        elif enemy_Atk == 20:
            time.sleep(0.6)
            print("The enemy got a natural 20")
            time.sleep(0.6)
            enemy_DamageN20 = (random.randint(1,10) + random.randint(1,10) + random.randint(1,8) + 4) * 2
            print(f"The enemy deals {enemy_DamageN20} damage")
            partyDict["LaeZel"]["HO"] -= enemy_DamageN20
            print(f"Player's Hp is {partyDict["LaeZel"]["HO"]}")

        elif enemy_Atk > partyDict["LaeZel"]["AC"]:
            time.sleep(0.6)
            print(f"Enemy rolled {enemy_Atk}")
            print("The enemy hits")
            time.sleep(0.6)
            enemy_Damage = random.randint(1,10) + random.randint(1,10) + random.randint(1,8) + 4
            print(f"The enemy deals {enemy_Damage} damage")
            partyDict["LaeZel"]["HO"] -= enemy_Damage
            print(f"Player's Hp is {partyDict["LaeZel"]["HO"]}")

        elif enemy_Atk < partyDict["LaeZel"]["AC"]:
            time.sleep(0.6)
            print(f"Enemy rolled {enemy_Atk}")
            print("The enemy missed")

        if enemyDict["Orc"]["HP"] <= 0:
            print("The player has won")
            break
        elif partyDict["LaeZel"]["HO"] <= 0:
            print("The player has lost")
            break

        if player_Atk == 1:
            time.sleep(0.6)
            print("The player got a natural 1")
            print("player misses")

        elif player_Atk == 20:
            time.sleep(0.6)
            print("The player got a natural 20")
            time.sleep(0.6)
            player_DamageN20 = (random.randint(1,6) + random.randint(1,6) + 3) * 2
            print(f"The player deals {player_DamageN20} damage")
            enemyDict["Orc"]["HP"] -= player_DamageN20
            print(f"enemy's Hp is {enemyDict["Orc"]["HP"]}")

        elif player_Atk > enemyDict["Orc"]["AC"]:
            time.sleep(0.6)
            print(f"Player rolled {player_Atk}")
            print("The player hits")
            time.sleep(0.6)
            player_Damage = random.randint(1,6) + random.randint(1,6) + 3
            print(f"The player deals {player_Damage} damage")
            enemyDict["Orc"]["HP"] -= player_Damage
            print(f"enemy's Hp is {enemyDict["Orc"]["HP"]}")

        elif player_Atk < enemyDict["Orc"]["AC"]:
            time.sleep(0.6)
            print(f"Player rolled {player_Atk}")
            print("The player missed")


elif Player_ini == enemy_ini:
    print("player go first")
    while enemyDict["Orc"]["HP"] > 0 or partyDict["LaeZel"]["HO"] > 0:
        time.sleep(0.6)
        player_Atk = random.randint(1, 20) + partyDict["LaeZel"]["AtkMod"]
        enemy_Atk = random.randint(1, 20) + enemyDict["Orc"]["AtkMod"]

        if enemyDict["Orc"]["HP"] <= 0:
            print("The player has won")
            break
        elif partyDict["LaeZel"]["HO"] <= 0:
            print("The player has lost")
            break

        if player_Atk == 1:
            time.sleep(0.6)
            print("The player got a natural 1")
            print("player misses")

        elif player_Atk == 20:
            time.sleep(0.6)
            print("The player got a natural 20")
            time.sleep(0.6)
            player_DamageN20 = (random.randint(1, 6) + random.randint(1, 6) + 3) * 2
            print(f"The player deals {player_DamageN20} damage")
            enemyDict["Orc"]["HP"] -= player_DamageN20
            print(f"enemy's Hp is {enemyDict["Orc"]["HP"]}")

        elif player_Atk > enemyDict["Orc"]["AC"]:
            time.sleep(0.6)
            print(f"Player rolled {player_Atk}")
            print("The player hits")
            time.sleep(0.6)
            player_Damage = random.randint(1, 6) + random.randint(1, 6) + 3
            print(f"The player deals {player_Damage} damage")
            enemyDict["Orc"]["HP"] -= player_Damage
            print(f"enemy's Hp is {enemyDict["Orc"]["HP"]}")

        elif player_Atk < enemyDict["Orc"]["AC"]:
            time.sleep(0.6)
            print(f"Player rolled {player_Atk}")
            print("The player missed")

        if enemyDict["Orc"]["HP"] <= 0:
            print("The player has won")
            break
        elif partyDict["LaeZel"]["HO"] <= 0:
            print("The player has lost")
            break

        if enemy_Atk == 1:
            time.sleep(0.6)
            print("The enemy got a natural 1")
            print("The enemy misses")

        elif enemy_Atk == 20:
            time.sleep(0.6)
            print("The enemy got a natural 20")
            time.sleep(0.6)
            enemy_DamageN20 = (random.randint(1, 10) + random.randint(1, 10) + random.randint(1, 8) + 4) * 2
            print(f"The enemy deals {enemy_DamageN20} damage")
            partyDict["LaeZel"]["HO"] -= enemy_DamageN20
            print(f"Player's Hp is {partyDict["LaeZel"]["HO"]}")

        elif enemy_Atk > partyDict["LaeZel"]["AC"]:
            time.sleep(0.6)
            print(f"Enemy rolled {enemy_Atk}")
            print("The enemy hits")
            time.sleep(0.6)
            enemy_Damage = random.randint(1, 10) + random.randint(1, 10) + random.randint(1, 8) + 4
            print(f"The enemy deals {enemy_Damage} damage")
            partyDict["LaeZel"]["HO"] -= enemy_Damage
            print(f"Player's Hp is {partyDict["LaeZel"]["HO"]}")

        elif enemy_Atk < partyDict["LaeZel"]["AC"]:
            time.sleep(0.6)
            print(f"Enemy rolled {enemy_Atk}")
            print("The enemy missed")


#3. If the attack hits, roll damage and subtract it from the target's hit points.

#4. The second in initiative rolls to attack (and rolls damage) afterwards.

#5. Repeat steps 2-5 until one of the characters is dead.
